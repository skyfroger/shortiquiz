...
Несколько идущих подряд вопросов `qmulti`,  `qcheck`, `qinput`, `qparson` и `qspot` можно сгруппировать с помощью блока `qgroup`.
...
С помощью элемента `qflip` вставляется карточка с двумя сторонами - вопросом и ответом. Пример разметки:
...
Вопрос и ответ разделяются внутри блока `.qflip` с помощью горизонтальной линии `---`.
